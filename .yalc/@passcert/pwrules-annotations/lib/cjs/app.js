...
exports.PasswordRulesParser = void 0;
const passwordBlocklist_1 = require("./data/passwordBlocklist");
const customCharacterData_1 = require("./data/customCharacterData");
const data_enum_1 = require("./data/data.enum");
const namedCharacterData_1 = require("./data/namedCharacterData");
const ruleData_1 = require("./data/ruleData");
class PasswordRulesParser {
    /**
     ...
     */
    parsePasswordRules(input, formatRulesForMinifiedVersion) {
        ...
        for (let rule of passwordRules) {
            switch (rule.name) {
                case data_enum_1.RuleName.MAX_CONSECUTIVE:
                ...
                case data_enum_1.RuleName.REQUIRED:
                    ...
                    rule.value.forEach(element => {
                        ...
                        if (this._hasCharRange(element)) {
                            ...
                            if (element.type === 'NamedCharacterData') {
                                allowedRule = Object.assign(new namedCharacterData_1.NamedCharacterData(element.name), element);
                            }
                            else if (element.type === 'CustomCharacterData') {
                                allowedRule = Object.assign(new customCharacterData_1.CustomCharacterData(element.characters), element);
                            }
                    ...
                    break;
                case data_enum_1.RuleName.ALLOWED:
                    rule.value.forEach(element => {
                    ...
                    break;
                case data_enum_1.RuleName.MIN_CLASSES:
                    actualMinClasses = rule.value;
                    break;
                case data_enum_1.RuleName.BLOCK_LIST:
                    newPasswordRules.push(new ruleData_1.RuleData(data_enum_1.RuleName.BLOCK_LIST, rule.value));
            }
        }
        ...
        if (!suppressCopyingRequiredToAllowed && !newAllowedValues.length) {
            newAllowedValues = [new namedCharacterData_1.NamedCharacterData(data_enum_1.Identifier.ASCII_PRINTABLE)];
        }
        if (newAllowedValues.length) {
            newPasswordRules.push(new ruleData_1.RuleData(data_enum_1.RuleName.ALLOWED, newAllowedValues));
        }
        if (minimumMaximumConsecutiveCharacters !== null) {
            newPasswordRules.push(new ruleData_1.RuleData(data_enum_1.RuleName.MAX_CONSECUTIVE, minimumMaximumConsecutiveCharacters));
        }
        if (maximumMinLength > 0) {
            newPasswordRules.push(new ruleData_1.RuleData(data_enum_1.RuleName.MIN_LENGTH, maximumMinLength));
        }
        if (minimumMaxLength !== null) {
            newPasswordRules.push(new ruleData_1.RuleData(data_enum_1.RuleName.MAX_LENGTH, minimumMaxLength));
        }
        if (actualMinClasses >= 1 && actualMinClasses <= 4) {
            newPasswordRules.push(new ruleData_1.RuleData(data_enum_1.RuleName.MIN_CLASSES, actualMinClasses));
        }
        else if (actualMinClasses < 1) {
            newPasswordRules.push(new ruleData_1.RuleData(data_enum_1.RuleName.MIN_CLASSES, 1));
        }
        else {
            newPasswordRules.push(new ruleData_1.RuleData(data_enum_1.RuleName.MIN_CLASSES, 4));
        }
        ...
        newPasswordRules.filter(rule => rule.name === data_enum_1.RuleName.REQUIRED)
            .forEach(rr => {
            rr.value.forEach(rt => {
     ...
     */
    _bitSetIndexForCharacter(c) {
        console.assert(c.length === 1);
        return c.codePointAt(0) - data_enum_1.SPACE_CODE_POINT;
    }
    /**
    ...
    */
    _characterAtBitSetIndex(index) {
        return String.fromCodePoint(index + data_enum_1.SPACE_CODE_POINT);
    }
    /**
     ...
     */
    _markBitsForNamedCharacterClass(bitSet, namedCharacterClass) {
        console.assert(bitSet instanceof Array);
        console.assert(namedCharacterClass.name !== data_enum_1.Identifier.UNICODE);
        console.assert(namedCharacterClass.name !== data_enum_1.Identifier.ASCII_PRINTABLE);
        if (namedCharacterClass.name === data_enum_1.Identifier.UPPER) {
            bitSet.fill(true, this._bitSetIndexForCharacter("A"), this._bitSetIndexForCharacter("Z") + 1);
        }
        else if (namedCharacterClass.name === data_enum_1.Identifier.LOWER) {
            bitSet.fill(true, this._bitSetIndexForCharacter("a"), this._bitSetIndexForCharacter("z") + 1);
        }
        else if (namedCharacterClass.name === data_enum_1.Identifier.DIGIT) {
            bitSet.fill(true, this._bitSetIndexForCharacter("0"), this._bitSetIndexForCharacter("9") + 1);
        }
        else if (namedCharacterClass.name === data_enum_1.Identifier.SPECIAL) {
            ...
            console.assert(false, data_enum_1.SHOULD_NOT_BE_REACHED, namedCharacterClass);
        }
    }
    /**
     ...
     */
    _markBitsForCustomCharacterClass(bitSet, customCharacterClass) {
     ...
     */
    _canonicalizedPropertyValues(propertyValues, keepCustomCharacterClassFormatCompliant) {
        let asciiPrintableBitSet = new Array("~".codePointAt(0) - " ".codePointAt(0) + 1);
        let hasCharRange = false;
        for (let propertyValue of propertyValues) {
            if (propertyValue instanceof namedCharacterData_1.NamedCharacterData) {
                if (propertyValue.name === data_enum_1.Identifier.UNICODE) {
                    return [new namedCharacterData_1.NamedCharacterData(data_enum_1.Identifier.UNICODE)];
                }
                if (propertyValue.name === data_enum_1.Identifier.ASCII_PRINTABLE) {
                    return [new namedCharacterData_1.NamedCharacterData(data_enum_1.Identifier.ASCII_PRINTABLE)];
                }
            ...
            else if (propertyValue instanceof customCharacterData_1.CustomCharacterData) {
        ...
        if (hasAllUpper) {
            let newNamedClass = propertyValues.filter(x => x.type === 'NamedCharacterData' && x.name === data_enum_1.Identifier.UPPER)[0];
            result.push(newNamedClass);
        }
        if (hasAllLower) {
            let newNamedClass = propertyValues.filter(x => x.type === 'NamedCharacterData' && x.name === data_enum_1.Identifier.LOWER)[0];
            result.push(newNamedClass);
        }
        if (hasAllDigits) {
            let newNamedClass = propertyValues.filter(x => x.type === 'NamedCharacterData' && x.name === data_enum_1.Identifier.DIGIT)[0];
            result.push(newNamedClass);
        }
        if (hasAllSpecial) {
            let newNamedClass = propertyValues.filter(x => x.type === 'NamedCharacterData' && x.name === data_enum_1.Identifier.SPECIAL)[0];
            result.push(newNamedClass);
        }
     ...
     */
    _markRangesAsUndefined(passwordRules) {
        for (let i = 0; i < passwordRules.length; i++) {
            if (passwordRules[i].name === data_enum_1.RuleName.REQUIRED) {
     ...
     */
    _indexOfNonWhitespaceCharacter(input, index = 0) {
     ...
     */
    _parseIdentifier(input, position) {
    ...
    _parseRange(input, position) {
        ...
        let result = 0;
        do {
            if (input[position] === data_enum_1.CHARACTER_RANGE_START_SENTINEL || this._isASCIIWhitespace(input[position])) {
                ++position;
                continue;
            }
            // if we see a whitespace or a comma, then we can push the result integer to the list. 
            // useful for integer > 9
            if (input[position] === data_enum_1.PROPERTY_VALUE_SEPARATOR) {
                seenRange.push(result);
                result = 0;
                ++position;
                continue;
            }
            if (input[position] === data_enum_1.CHARACTER_RANGE_END_SENTINEL) {
                seenRange.push(result);
                result = 0;
                break;
            }
     ...
     */
    _isValidRequiredOrAllowedPropertyValueIdentifier(identifier) {
        return identifier && Object.values(data_enum_1.Identifier).includes(identifier.toLowerCase());
    }
    /**
     ...
     * @returns True if it is a valid identifier. False if it is not a valid identifier.
     */
    _isValidBlockListPropertyValueIdentifier(identifier) {
        return identifier && Object.values(data_enum_1.BlockListIdentifier).includes(identifier.toLowerCase());
    }
    /**
     ...
     */
    _parseCustomCharacterClass(input, position) {
        console.assert(position >= 0);
        console.assert(position < input.length);
        console.assert(input[position] === data_enum_1.CHARACTER_CLASS_START_SENTINEL);
        let length = input.length;
        ++position;
        if (position >= length) {
        ...
        do {
            ...
            result.push(c);
            ++position;
            if (c === data_enum_1.CHARACTER_CLASS_END_SENTINEL) {
                break;
            }
        } while (position < length);
        if (position < length && input[position] !== data_enum_1.CHARACTER_CLASS_END_SENTINEL || position == length && input[position - 1] == data_enum_1.CHARACTER_CLASS_END_SENTINEL) {
            // Fix up result; we over consumed.
            result.pop();
            return [result, position];
        }
        if (position < length && input[position] == data_enum_1.CHARACTER_CLASS_END_SENTINEL) {
    ...
    _parsePasswordRequiredOrAllowedPropertyValue(input, position) {
        ...
        while (true) {
            if (this._isIdentifierCharacter(input[position])) {
                ...
                propertyValues.push(new namedCharacterData_1.NamedCharacterData(propertyValue));
                position = this._indexOfNonWhitespaceCharacter(input, position);
                if (input[position] === data_enum_1.CHARACTER_RANGE_START_SENTINEL) {
                    ...
                    if (rangeChars.length !== 2 || (rangeChars[0] === '0' && rangeChars[1] === '0')) {
                        ...
                        let lastPushedNamedClass = propertyValues[propertyValues.length - 1];
                        lastPushedNamedClass.minChars = rangeChars[0];
                        lastPushedNamedClass.maxChars = rangeChars[1];
                    }
                }
            }
            else if (input[position] === data_enum_1.CHARACTER_CLASS_START_SENTINEL) {
                ...
                if (propertyValueArray && propertyValueArray.length) {
                    propertyValues.push(new customCharacterData_1.CustomCharacterData(propertyValueArray));
                }
                if (input[position] === data_enum_1.CHARACTER_RANGE_START_SENTINEL) {
                    position = this._indexOfNonWhitespaceCharacter(input, position);
                    let [range, index] = this._parseRange(input, position);
            ...
            position = this._indexOfNonWhitespaceCharacter(input, position);
            if (position >= length || input[position] === data_enum_1.PROPERTY_SEPARATOR) {
                break;
            }
            if (input[position] === data_enum_1.PROPERTY_VALUE_SEPARATOR) {
     ...
     */
    _parseBlockListPropertyValue(input, position) {
        let propertyValues = [];
        if (this._isIdentifierCharacter(input[position])) {
            let identifierStartPosition = position;
            let [propertyValue, index] = this._parseIdentifier(input, position);
            ...
            if (propertyValue === 'default') {
                const passwordBlocklist = passwordBlocklist_1.PasswordBlocklist.getInstance();
     ...
     */
    _parsePasswordRule(input, position) {
        console.assert(position >= 0);
        console.assert(position < input.length);
        console.assert(this._isIdentifierCharacter(input[position]));
        let length = input.length;
        ...
        if (!Object.values(data_enum_1.RuleName).includes(identifier)) {
        ...
        if (input[position] !== data_enum_1.PROPERTY_VALUE_START_SENTINEL) {
            console.error("Failed to find start of property value: " + input.substr(position));
            return [null, position];
        }
        ...
        // Empty value
        if (position >= length || input[position] === data_enum_1.PROPERTY_SEPARATOR) {
            return [new ruleData_1.RuleData(property.name, property.propValue), position];
        }
        switch (identifier) {
            case data_enum_1.RuleName.ALLOWED:
            case data_enum_1.RuleName.REQUIRED: {
        ...
        console.assert(false, data_enum_1.SHOULD_NOT_BE_REACHED);
    }
    /**
     ...
     */
    _parseMinLengthMaxLengthPropertyValue(input, position) {
     ...
     * @returns The value for the rule and the last position analyzed.
     */
    _parseMaxConsecutivePropertyValue(input, position) {
        return this._parseInteger(input, position);
    }
    /**
     ...
     * @returns The value for the rule and the last position analyzed.
     */
    _parseMinClassesPropertyValue(input, position) {
        return this._parseInteger(input, position);
    }
    /**
    ...
    _parseInteger(input, position) {
        ...
        do {
            result = 10 * result + parseInt(input[position], 10);
            ++position;
        } while (position < length && input[position] !== data_enum_1.PROPERTY_SEPARATOR && this._isASCIIDigit(input[position]));
        if (position >= length || input[position] === data_enum_1.PROPERTY_SEPARATOR) {
            return [result, position];
        }
     ...
     */
    _parsePasswordRulesInternal(input) {
        ...
        while (position < length) {
            ...
            if (input[position] === data_enum_1.PROPERTY_SEPARATOR) {
                position = this._indexOfNonWhitespaceCharacter(input, position + 1);
                if (position >= length) {
                    return parsedProperties;
                }
     ...
     */
    _fixRangeIncoherences(passwordRules, classesSeen) {
        ...
        // Thus, we need to remove the ranges. If there is only one required rule with all 4 classes, this class will convert to ascii-printable.
            ...
            let numRequiredRules = passwordRules.filter(rule => rule.name === data_enum_1.RuleName.REQUIRED).length;
            ...
            if (numRequiredRules > 1) {
                ...
                let ruleIndex = passwordRules.findIndex(rule => rule.name === data_enum_1.RuleName.REQUIRED);
                passwordRules[ruleIndex].value = [new namedCharacterData_1.NamedCharacterData(data_enum_1.Identifier.ASCII_PRINTABLE)];
            }
        }
    }
}
exports.PasswordRulesParser = PasswordRulesParser;
